...
function SelectorPage({
  ...
  const isMarvel = gameType === 'marvel';
  
  return (
    <div style={{ maxWidth: '100vw', overflowX: 'hidden', padding: '0.5rem' }}>
      <h1>{isMarvel ? 'Marvel Champions Hero Roller' : 'Arkham Investigator Roller'}</h1>
      ...
      <div className="cardContainer">
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
          {current ? (
            <div className="mobileCard" style={{ border: '1px solid #aaa', borderRadius: 8, padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              {isMarvel ? (
                current.code ? (
                  <img src={`https://marvelcdb.com${current.imagesrc}`} alt={current.name} style={{ borderRadius: 10, boxShadow: '0 2px 16px #0003', marginBottom: 12, background: '#222' }} />
                ) : (
              ...
              <br/>
              {!isMarvel && <div style={{ fontWeight: 'bold', fontSize: 18, margin: '0.5rem 0' }}>{FACTION_ICONS[current.faction_code] || null}</div>}
              {isMarvel && current.traits && (
                <p style={{ margin: '0.5rem 0', textAlign: 'center', fontStyle: 'italic' }}>Traits: {current.traits}</p>
              )}
            ...
            <div style={{ textAlign: 'center', padding: '1rem' }}>
              <h2>No more {isMarvel ? 'heroes' : 'investigators'} in queue.</h2>
            </div>
          )}
        </div>
        <div style={{ width: '100%', maxWidth: '100vw', background: '#232323', borderRadius: 10, padding: '1rem', marginTop: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', boxShadow: '0 2px 12px #0003' }}>
          <h3 style={{ fontSize: '1.3em', marginBottom: '0.5em', textAlign: 'center' }}>Accepted {isMarvel ? 'Heroes' : 'Investigators'} ({accepted.length}):</h3>
            ...
            {accepted.map(card => (
              <li key={card.code} className="cardPreviewWrapper" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                {isMarvel ? (
                  card.code ? (
                    <>
                      <img src={`https://marvelcdb.com${card.imagesrc}`} alt={card.name} style={{ borderRadius: 6, background: '#222' }} />
                      <div className="previewCard">
                        <img src={`https://marvelcdb.com${card.imagesrc}`} alt={card.name} />
                        <p>{card.name}</p>
                      </div>
                    </>
                  ) : (
